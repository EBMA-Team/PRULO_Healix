...
This analysis links to the [manuscript](https://doi.org/10.1101/2024.06.13.24307996) of the Healix BR product. The dataset is derived from the PRULO registry snapshot and live tables. A protocol has been previously prepared for the registry [@scholes2023].
...
Packages were loaded initially with *pacman* package (v`{r} utils::packageVersion("pacman")`) [@pacman] Citations were applied to each library at first use in the text. <!--# Reorder these in order of general appearance -->
...
Access to PRULO datasets was pre-authorised using the *gargle* package (v`{r} utils::packageVersion("gargle")`) [@gargle] and *googledrive* (v`{r} utils::packageVersion("googledrive")`) [@googledrive].
...
Data was retrieved using *googlesheets4* (v`{r} utils::packageVersion("googlesheets4")`) [@googlesheets4] to retrieve live database tables. Source files were specified and stored as global variables to call on in further functions.
...
A static registry snapshot was retrieved using the pre-specified function (see *Functions for Processing)* and formatted using *openxlsx* [@openxlsx2] based on the fixed date of preparation of the snapshot (31-Mar-2024) and using *tidyverse* (v`{r} utils::packageVersion("tidyverse")`) [@tidyverse]syntax and associated packages. Date columns were prepared for further analysis using *lubridate* (v`{r} utils::packageVersion("lubridate")`) [@lubridate].
...
An additional export of account data was prepared and imported to the workspace using the *readr* package (v`{r} utils::packageVersion("readr")`) [@readr], as well as *tidyverse* syntax and *stringr* (v`{r} utils::packageVersion("stringr")`) [@stringr], to categorise text fields.
...
x <- drive_get(
as_id("1Xo58d7b6NJOVIxPbYmlCrDRiVGhmKan8")
)
...
Identify cases receiving the suture of interest. Cases were identified by SKUs identified from the [SKU database](#0 "Database of implants to be tracked for Depuy-Mitek in PRULO database") maintained as part of implant tracking within the registry. Cases were not restricted by available follow up.
...
Data manipulation (add columns and filter tables based on column values) was performed with *tidyverse* and converted to display format using *gt* (v`{r} utils::packageVersion(pkg = "gt")`) [@gt].

Table 1: Summary of SKUs (Reference) used to identify cases of interest from PRULO registry

```{r, Hardware of Interest}
...
ImplantTable2 |> gt(rowname_col = "row")
```
...
## Slice inputs for columns and rows
#| label: "Identify sample of interest"
#| code-summary: "line up input frames"
...
## Slice inputs for columns and rows
#| label: "Remove non-index procedures"
#| code-summary: "line up input frames"
...
Table 2: Summary of key variable definitions in the analysis

| Category | Variable | Comments | Citation |
|------------------|------------------|--------------------|------------------|
| Patient Characteristics | Insurance Status | Recode from account data from practice management system to insurance status |  |
...
Data was sourced directly from the PRULO clinical registry.
...
Table 3: Biases in analysis of observational cohort of a clinical registry

| Bias | Definition | Source | Mitigation |
|------------------|------------------|------------------|-------------------|
| Misclassification | Treatment record labelled into incorrect cohort. PROMs package not aligned to | [@Benchimol2015] | Clinical notes reviewed by experienced reviewer and matched to ICD10 code by definition. |
...
Missingness was assessed with visualisation and table functions in the *naniar* package (v`{r} utils::packageVersion("naniar")`) [@naniar] and compiled into figures using *patchwork* (v`{r} utils::packageVersion("patchwork")`) [@patchwork].
...
MissFig4 <- MasterPROM |>
  dplyr::select(QDASH,
                TimePoint) |>  gg_miss_var(show_pct = TRUE,
                                        facet = TimePoint) + labs(title = "Patient Reported Outcomes - 1")


MissFig5 <- MasterPROMWORC |>
  dplyr::select(WORCNorm,
                TimePoint) |>  gg_miss_var(show_pct = TRUE,
                                        facet = TimePoint) + labs(title = "Patient Reported Outcomes - 2")
...
The data tables were reduced to the required columns (PROMs and adjunct columns) in preparation for multiple imputation using chained equations [@white2010] with the *mice* package (v`{r} utils::packageVersion("mice")`) [@mice]. One patient with bilateral records in the sample had one field (EducationLevel_Preop) mirrored from one side record to the other, where it was missing. Character fields were converted to factors and the dataset was filtered to those cases that were eligible for 12months followup.
...
A predictor matrix was generated to specify the combination of variables to be drawn on for the imputation of each column in the dataset. In addition, a *method* matrix was created to specify varying univariate imputations to account for the multilevel nature of the dataset \[citation\]. Patient level variables (education, sex, bilateralstatus) were imputed as level-2 variables and the PROMs columns treated as level-1 variables.
...
A flow chart of individual treatment episodes (treatments) was generated using the *consort* package (v`{r} utils::packageVersion("consort")`)[@consort] and prepared for display with the *knitr* package (v`{r} utils::packageVersion("knitr")`) [@knitr].
...
Figure 2: Flow chart of treatment record inclusion and separation into cohorts for analysis. mFU - Minimum follow up (30 days).

The table below summarises patient diagnoses in the PRULO registry.

Table 4: Summary of diagnoses by ICD-10 code
...
TableData <- Mastersheet2 |>
  ...
  slice_head(n = 5) |> gt::gt(
             caption = "Table 4: Summary of Sample Diagnoses by ICD-10"
             )
...
Patient characteristics for cases receiving the anchor of interest are summarised in Table 5.
...
knitr::knit_print(PatientChar)

```

### Record \[14.1\] Pathology characteristics

Pathology characteristics for cases receiving the anchor of interest are summarised in Table 6.
...
knitr::knit_print(ManageSum)
```

### Record \[14.3\] Follow up

The sample overall had a mean follow up of `{r} mean(Mastersheet2$TreatDuration)` months.

Table 8: Summary of case followup (months) for the sample included for analysis
...
knitr::knit_print(FollowupTab)
```

```{r Follow up summary}

## Summarise follow up duration
#| label: Follow up summary
#| include: false
#| code-summary: "Summarise follow up duration in figure"
#| warning: false
#| fig-cap: Figure 3 - Summary of follow up duration
...
Figure 3: Summary of follow up duration for the included sample.
...
A summary of the adverse events observed is included in Table 10.
...
knitr::knit_print(ComplicSum)
```

### Record \[15.4\] Patient-reported outcome measures

The QuickDASH total score and WORC Normalised Index, as well as Question 3 of the Physical sub-scale of the WORC were visualised using the *ggdist* (v`{r} utils::packageVersion("ggdist")`) [@ggdist] and *ggplot2* (v`{r} utils::packageVersion("ggplot2")`) [@ggplot2]packages. Plots were arranged using the *patchwork* package.

```{r}

## Plot PROMs trajectories
#| label: PROMs complete case analysis
#| code-summary: "Summary of PROMs trajectories - complete case analysis"
#| warning: false
...
Figure 6: Summary of patient reported outcomes trajectories by timepoint

## Record \[16\] Main results

The imputed datasets for QDASH and WORC were modeled with a linear mixed effects model in *lme4* (v`{r} utils::packageVersion("lme4")`) [@lme4]and summarised with *broom.mixed* (v`{r} utils::packageVersion("broom.mixed")`) [@broom.mixed]. Up to a 38.7 point improvement in QuickDASH total score was observed (Table 11), as well as 47.1 and 54 point improvements in WORC Index Normalised and WORC Physical Question3 respectively (Table 12). Distributions of model-predicted results illustrated variability in recovery trajectories within all PROMs measures (Figure 7).
...
QDASHfimpsum <- tbl_regression(
  ...
  pvalue_fun = function(x) style_pvalue(x, digits = 3))  %>% modify_caption("Table tt: ")

knitr::knit_print(QDASHfimpsum)
...
WORCfitsum <- tbl_merge(tbls = list(WORCNormfitsum, WORCQ3fitsum),
                        tab_spanner = c("Normalised Index", "Physical Q3")
) %>% modify_caption("Table zz: Summary of pooled linear model results WORC")

knitr::knit_print(WORCfitsum)
```
...
Table 13a: Summary of model-predicted QuickDASH by TimePoint
...
as_flex_table(TableQDASHImp)
```

Table 13b: Summary of model-predicted WORC by TimePoint
...
as_flex_table(WORCfitsum)

```

```{r QDash plot - imputed 2}
...
Figure 7: Model predicted PROMs trajectories across time points.

## Record \[17\] Sensitivity analyses

Based on the distribution changes in QDASH and WORC over time, a sensitivity analysis was performed on the model structure using the complete case dataset. A comparison was made between quantile regression using the *quantreg* package (v`{r} utils::packageVersion("quantreg")`) [@quantreg]and an ordinary least squares linear model from the *rstats* package (v`{r} utils::packageVersion("stats")`) [@stats] and a linear mixed effects model with the *lme4* package (v`{r} utils::packageVersion("lme4")`) [@lme4]. Results were tabulated using the *modelsummary* package (v`{r} utils::packageVersion("modelsummary")`) [@modelsummary] as rq models are not supported in *gtsummary*.
...
The comparison between models revealed an underestimate of the difference in 12month score to preoperative baseline of {round((60.0 - 52.3),1)} points for the QuickDASH ({round(((60.0 - 52.3)/52.3),3)*100}%) in the mixed effects linear model, compared to the quantile regression (50th percentile).
